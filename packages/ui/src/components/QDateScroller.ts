...
export default defineLegacyComponent({
  ...
  computed: {
    ...
    displayDate() {
      ...
      // year only
      if (this.noDays === true && this.noMonths === true)
        return this.yearFormatter(this.timestamp, this.shortYearLabel);
      // month only
      if (this.noDays === true && this.noYears === true)
        return this.monthFormatter(this.timestamp, this.shortMonthLabel);
      // day only
      if (this.noMonths === true && this.noYears === true)
        return this.dayFormatter(this.timestamp, this.shortDayLabel);
      // month and year
  ...
  methods: {
    ...
    handleDisabledLists() {
      ...
      this.disabledMonths.forEach((m) =>
        this.disabledMonthsList.push(padNumber(parseInt(m, 10), 2)),
      );
