...
// "transparent" belongs here too because it is a valid CSS color token.
...
const QUASAR_PALETTE: Record<string, string> = {
  primary: "#1976d2",
  secondary: "#26a69a",
  accent: "#9c27b0",
  "dark-page": "#121212",
...
const cssColorNames = new Set(CSS_COLOR_NAMES);

export const QColorizeMixin = defineLegacyComponent({
  name: "QColorizeMixin",

  methods: {
    isNamedCssColor(color: string | undefined) {
      return color !== void 0 && cssColorNames.has(color.toLowerCase());
    },

    isCssColor(color: string | undefined) {
      return color !== void 0 && (/^(#|(rgb|hsl)a?\()/.test(color) || this.isNamedCssColor(color));
    },

    isCssVar(color: string | undefined) {
      return color !== void 0 && (color.startsWith("--") || color.startsWith("var("));
    },

    calculateColor(color: string | undefined, defaultColor = "black") {
      if (color === void 0) {
        return this.calculateColor(defaultColor);
      }

      const colorName = color.toString().trim();

      if (this.isCssColor(colorName)) {
        return colorName;
      }
      ...
      if (colorName.startsWith("--")) {
        return `var(${colorName}, ${defaultColor})`;
      }

      return QUASAR_PALETTE[colorName] ?? `var(--q-${colorName}, ${defaultColor})`;
    },

    isValidCssColor(color: string | undefined) {
