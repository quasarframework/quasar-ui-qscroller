...
export default defineLegacyComponent({
  ...
  methods: {
          ...
          if (scrollToEl && scrollToEl.nodeName === "BUTTON" && scrollToEl.innerText.length > 0) {
            ...
            this.scrollTimer = setTimeout(() => {
              ...
              const items = this.items.filter(
                (v) =>
                  v.value === scrollToEl.innerText ||
                  (v.label !== undefined && v.label === scrollToEl.innerText),
              );
    ...
    adjustColumnPadding() {
      ...
      const getPadding = () => {
        self.height = parseInt(
          window.getComputedStyle(self.$el.parentElement, null).getPropertyValue("height"),
          10,
        );
