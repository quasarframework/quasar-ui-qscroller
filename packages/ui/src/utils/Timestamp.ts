/* from QCalendar - figure out a way to keep this in sync */
...
export function getWorkWeek(timestamp) {
  ...
  const weekDiff = (weekday.getTime() - firstThursday.getTime()) / MILLISECONDS_IN_WEEK;
...
export function createNativeLocaleFormatter(locale, getOptions) {
  ...
  return (timestamp, short) => {
    try {
      ...
      console.error(`Intl.DateTimeFormat: ${message} -> ${getDateTime(timestamp)}`);
      return "";
    }
  };
}
...
export function makeDateTime(timestamp) {
  return new Date(
    Date.UTC(timestamp.year, timestamp.month - 1, timestamp.day, timestamp.hour, timestamp.minute),
  );
}
