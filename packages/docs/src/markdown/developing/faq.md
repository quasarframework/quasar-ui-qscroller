...
:::details Q. How do I type an App Extension-registered QScroller template ref?
...
import type { QScroller } from '@quasar/quasar-ui-qscroller'

const scrollerRef = useTemplateRef<QScroller>('scroller')
```

A normal value import of `QScroller` creates a local component binding in `<script setup>` and takes precedence over the globally registered component. Use a normal import only when you intend to use the UI component directly, and follow the manual boot file installation path instead of registering it through both paths.
